...
class BiDAF(object):
	...
	def __init__(self, config, seqlen, quelen, wordic):
		...
		self.ans_s = tf.softmax(self.logit_s, 1)
		self.ans_e = tf.softmax(self.logit_e, 1)
	...
	def optimize(self, config, logit_s, logit_e, y1, y2):
			loss_1 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
																						logits=logit_s, labels=y1))
			loss_2 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
																						logits=logit_e, labels=y2))
			self.loss = loss_1 + loss_2
			now = time.localtime()
			print("variable initializing ({}:{}:{})".format(now.tm_hour, now.tm_min, now.tm_sec))
			tvars = tf.trainable_variables()
			end = time.localtime()
			spending_time = end.tm_hour*3600 + end.tm_min*60 + end.tm_sec - (now.tm_hour*3600 + now.tm_min*60 + now.tm_sec)
			print("spending time : {}".format(spending_time))
